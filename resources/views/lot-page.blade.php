...
@section('content')
    ...
    <div class="d-flex flex-row justify-content-around">
        ...
        <div class="px-5 text-end">
            ...
            @if(isset($lot->buy_now_price))
                <p class="text-success">Buy now for {{$lot->buy_now_price}}</p>
            @endif
            <p>{{$bids->count()}} bids (0 yours)</p>
            <form action="{{route('makeBid', $lot->id)}}" method="POST" class="row align-items-center bg-success p-3">
                @csrf
    ...
    <ul class="list-group">
        @foreach($bids as $bid)
            <li class="list-group-item">User make bid {{$bid->bid_price}} ({{$bid->created_at}})</li>
        @endforeach
    </ul>
